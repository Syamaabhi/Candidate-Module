...
//   try {
    ...
    const [isApplicationModalOpen, setIsApplicationModalOpen] = React.useState(false);
     const [isApplicationModalOpenu, setIsApplicationModalOpenu] = React.useState(false);
 ...
 React.useEffect(() => {
  ...
  const idFromUrl = params.get("user_id"); // e.g., "25"
  setUserId(idFromUrl);
    fetchJobs();
...
if (userId) {
    ...
    const res = await fetch("http://localhost/candidatemodule/public/api/registerjobs.php", {
      method: "POST",
 ...
 setIsApplicationModalOpenu(true);
  } catch (err) {
    ...
    return (
      ...
      />
        ...
        >
         ...
         {/* Application Modal */}
        <Modal 
            isOpen={isApplicationModalOpenu} 
            onClose={() => setIsApplicationModalOpenu(false)}
           isSubmitting={isSubmitting}

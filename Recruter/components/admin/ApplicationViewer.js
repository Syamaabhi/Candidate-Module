...
//reference 
// const sendInterviewMail = (candidate) => {
//   if (!candidate?.email) {
//     alert("Candidate email not found");
//     return;
//   }

//   const subject = encodeURIComponent("Interview Invitation - SkillMatrix");
//   const body = encodeURIComponent(
// `Hello ${candidate.name || "Candidate"},

// Congratulations! You have been shortlisted for the next round.

// 📅 Interview Date: ${candidate.interview_date || "To be scheduled"}
// ⏰ Time: ${candidate.interview_time || "Will be shared soon"}
// 🔗 Meeting Link: ${candidate.meeting_link || "https://meet.google.com"}
...
//eference
const sendInterviewMail = () => {
   
     const candidate = {
    name: "Rahul Sharma",
    email: "rahul@example.com",
    interview_date: "25 Feb 2026",
...
//mm
  ...
  return (
    <div className="space-y-6" data-name="application-viewer">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border overflow-x-auto max-h-[600px]">
          <table className="min-w-full divide-y text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-8 py-4 text-left text-xs">Candidate</th>
                <th className="px-8 py-4 text-left text-xs">Contact</th>
                <th className="px-8 py-4 text-left text-xs">Applied</th>
                <th className="px-8 py-4 text-left text-xs">Resume download</th>
                
                <th className="px-8 py-4 text-left text-xs">Mail Send</th>
              </tr>
            </thead>
            <tbody>
  ...
  {app.objectData.resume_upload ? (
    <a
      href={`http://localhost/noww/download.php?file=${encodeURIComponent(app.objectData.resume_upload)}`}
      target="_blank"
...
<button
  onClick={() =>
    sendInterviewMail({
      name: app.objectData.applicant_name,
      email: app.objectData.applicant_email,
      interview_date: app.objectData.interview_date,
      interview_time: app.objectData.interview_time,
      meeting_link: app.objectData.meeting_link
    })
  }
